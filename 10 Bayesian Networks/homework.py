...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        for assignment in self.assignments:
            for prob in self.probability_table:
                if assignment == prob:
                    total = 0
                    nProb = 0
                    for i in self.probability_table.get(prob):
                        nProb = nProb + 1
                        total = total + i
                    self.marginal_probabilities[self.get_assignment_index(assignment)] = total / nProb
        ...
        for var in self.variables:
            ...
            if len(var.parents) is not 0:
                ...
                for parent in var.parents:
                    parentstate.append(values[parent.name])
                statetuple = tuple(parentstate)
                state = var.probability_table[statetuple]

                if values[name] == "true":
                    someList.append(state[1])
                else:
                    someList.append(state[0])
            else:
                for prob in var.probability_table:
                    if values[name] == "true":
                        someList.append(var.probability_table[prob][1])
                    else:
                        someList.append(var.probability_table[prob][0])
        for x in someList:
            if someList.index(x) == 0:
                jointprobability = x
            else:
                jointprobability *= x
